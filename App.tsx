...
const App: React.FC = () => {
  ...
  const toggleGroupAi = useCallback((contactId: string) => {
      setContacts(prev => prev.map(c => c.id === contactId ? { ...c, hasAiActive: !c.hasAiActive } : c));
  }, []);
  ...
  const handleAddContact = useCallback((name: string, id?: string) => {
      ...
      if (contacts.some(c => c.id === newId)) { setActiveContactId(newId); setCurrentTab('chat'); return; }
      const newContact: Contact = { id: newId, name: name, avatar: `https://picsum.photos/seed/${newId}/200`, lastMessage: '我们已经是好友了，开始聊天吧', lastMessageTime: Date.now(), isAi: false, peerId: isPeerId ? id : undefined };
      if (isPeerId) { p2pService.connectToPeer(id!); }
      setContacts(prev => [...prev, newContact]);
      setMessagesMap(prev => ({ ...prev, [newId]: [{ id: Date.now().toString(), content: '我通过了你的朋友验证请求，现在我们可以开始聊天了', senderId: 'system', timestamp: Date.now(), type: MessageType.SYSTEM }] }));
      setActiveContactId(newId); setCurrentTab('chat');
  }, [contacts]);
  ...
  const handleSendMessage = useCallback(async (content: string, type: MessageType = MessageType.TEXT, extra: { duration?: number, fileName?: string, fileSize?: string } = {}) => {
    ...
    if (currentContact?.peerId) {
        ...
        setTimeout(() => setMessagesMap(prev => ({ ...prev, [activeContactId]: prev[activeContactId].map(m => m.id === newMessageId ? { ...m, status: 'sent' } : m) })), 500);
        return; 
    }
    ...
    setTimeout(() => setMessagesMap(prev => ({ ...prev, [activeContactId]: prev[activeContactId].map(m => m.id === newMessageId ? { ...m, status: 'sent' } : m) })), 800);
    ...
    if (shouldAiReply) {
      ...
      try {
        ...
        setMessagesMap(prev => {
            const updatedMessages = prev[activeContactId].map(msg => msg.id === newMessageId ? { ...msg, status: 'read' as const } : msg);
  ...
  let mainContent;
  if (currentTab === 'moments') mainContent = <div className="flex flex-col w-full h-full relative"><div className="md:hidden absolute top-4 left-4 z-50"><button onClick={() => handleTabChange('chat')} className="bg-gray-200 p-2 rounded-full text-gray-600">← 返回</button></div><Moments currentUser={currentUser} moments={moments} onAddMoment={handleAddMoment} onAddComment={handleAddComment} onLikeMoment={handleLikeMoment}/></div>;
  else if (currentTab === 'stickers') mainContent = <StickerManager stickers={customStickers} onAddSticker={handleAddSticker} onDeleteSticker={handleDeleteSticker} onReorderStickers={handleReorderStickers}/>;
  else if (activeContactId === 'new_friends') mainContent = <NewFriends onAddContact={handleAddContact} />;
  else if (activeContactId === 'group_create') mainContent = <GroupCreator contacts={contacts} currentUser={currentUser} onCreateGroup={handleCreateGroup} onCancel={() => { setActiveContactId(''); setCurrentTab('chat'); }}/>;
  else if (activeContact) mainContent = <div className="flex flex-col w-full h-full relative"><div className="md:hidden absolute top-4 left-4 z-50"><button onClick={() => setActiveContactId('')} className="bg-gray-200 p-2 rounded-full text-gray-600">← 返回</button></div><ChatWindow activeContact={activeContact} messages={currentMessages} currentUserAvatar={currentUser.avatar} onSendMessage={handleSendMessage} onDeleteMessage={handleDeleteMessage} onToggleGroupAi={toggleGroupAi} onAddMember={handleAddMember} onUpdateContactAvatar={handleUpdateContactAvatar} onDeleteChat={handleDeleteChat} isTyping={isTyping} stickers={customStickers}/></div>;
  else mainContent = <div className="flex items-center justify-center w-full h-full text-gray-400 bg-[#f5f5f5]"><div className="text-center"><img src="https://picsum.photos/id/2/100/100" className="w-20 h-20 mx-auto mb-4 opacity-20 grayscale rounded-full" alt="Logo"/><p>选择一个聊天开始发送消息</p></div></div>;
